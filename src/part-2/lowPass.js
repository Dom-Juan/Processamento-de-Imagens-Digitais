const lowPass = (imgData, cutSize) => {
  ...
  let imgDataLowPass = context1.createImageData(imgData.width, imgData.height);
  imgDataLowPass.data.set(new Uint8ClampedArray(pixels));

  return imgDataLowPass;
};

$("#btn-low-pass").on("click", () => {
  console.log("** Passa baixa **");
  $("#low-pass-result").empty();
  $("#cut-value").empty();
  ...
  if ($("#cut-value").val() === undefined || $("#cut-value").val() === null || isNaN($("#cut-value").val()))
    $("#low-pass-result").append("Digite um valor valido para o corte!!!");
  else {
    

    let result = lowPass(imgData, t);
    ...
    t = $("#low-pass-result").empty();
    $("#low-pass-result").append("Sucesso ao realizar a operação!");
  }
});